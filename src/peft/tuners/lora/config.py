...
class MLoraConfig:
    use_exp: bool = field(
        default=False, metadata={"help": "If true, W' = W * exp(BA)"}
    )
    use_weight_norm: bool = field(
        default=False, metadata={"help": "If true, W' is normalized to match the Frobenius norm of W"}
    )
    fix_a: bool = field(
        default=False, metadata={"help": "If true, W' = W * exp(B1)"}
    )
    fix_b: bool = field(
        default=False, metadata={"help": "If true, W' = W * exp(1A)"}
    )
...
class LoraConfig(PeftConfig):
    ...
    Args:
        ...
        mlora (`bool` | `Literal["vanilla", "exp"]`):
            If specified, uses multiplicative LoRA. Can be 'vanilla' or 'exp'
    """
    ...
    init_lora_weights: (
        bool | Literal["gaussian", "eva", "olora", "pissa", "pissa_niter_[number of iters]", "corda", "loftq", "ones", "zeros"]
    ) = field(
        default=True,
        metadata={
            "help": (
                ...
                "Passing `'ones'` results in 1./sqrt(r) initialization for multiplicative lora. "
                "Passing `'zeros'` results in 0. initialization"
            ),
        },
    )
    ...
    use_mlora: bool = field(
        default=False,
        metadata={"help": "Whether to use multiplicative LoRA."},
    )
    mlora_config: Optional[MLoraConfig] = field(
        default=None,
