...
from .config import LoraConfig, MLoraConfig
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            if self.lora_bias[active_adapter]:
                ...
                result = F.linear(x, adapted_weight, self.base_layer.bias)
        elif self.merged:
            result = self.base_layer(x, *args, **kwargs)
        else:
            result = self.base_layer(x, *args, **kwargs)
...
class MultiheadAttention(nn.Module, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._active_adapter = adapter_name
        self.update_layer(adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, use_mlora, mlora_config)
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, use_mlora=False, mlora_config=None) -> None:
        # store values that then will be transfered to the Linear out_proj layer by super().update_layer
        if r <= 0:
            raise ValueError(f"`r` should be a positive integer value but the value passed is {r}")
        ...
        # Set scale right before the forward pass
        self.set_adapter(self.active_adapters)

        # update out_proj too
        # Note: LoRA is applied to both in_proj and out_proj. There is currently no way to only specify one of them.
        self.base_layer.out_proj.update_layer(
            adapter_name,
            r,
            lora_alpha=lora_alpha,
            lora_dropout=lora_dropout,
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            # Handle in_proj weight multiplication
            in_delta_weight = self.get_delta_weight(active_adapter)
            out_delta_weight = self.base_layer.out_proj.get_delta_weight(active_adapter)
            
            # Instead of modifying weights in-place, use a functional approach
            base_layer = self.get_base_layer()
            original_in_weight = base_layer.in_proj_weight
            original_out_weight = base_layer.out_proj.get_base_layer().weight
            
            # Create adapted weights, maintaining computational graph
            adapted_in_weight = original_in_weight * in_delta_weight
            adapted_out_weight = original_out_weight * out_delta_weight
            
            if self.mlora_config.use_weight_norm:
                # Normalize the in_proj weights
                fro_norm_adapted_in_weight = torch.norm(adapted_in_weight, p="fro")
                fro_norm_base_in_weight = torch.norm(original_in_weight, p="fro")
                adapted_in_weight = adapted_in_weight * fro_norm_base_in_weight / (fro_norm_adapted_in_weight + 1e-6)
                
                # Normalize the out_proj weights
                fro_norm_adapted_out_weight = torch.norm(adapted_out_weight, p="fro")
                fro_norm_base_out_weight = torch.norm(original_out_weight, p="fro")
                adapted_out_weight = adapted_out_weight * fro_norm_base_out_weight / (fro_norm_adapted_out_weight + 1e-6)
            
            # Temporarily replace the weights with the adapted versions
            try:
                # Store original weights
                temp_in_weight = base_layer.in_proj_weight.data.clone()
                temp_out_weight = base_layer.out_proj.get_base_layer().weight.data.clone()
                
                # Replace with adapted weights
                base_layer.in_proj_weight.data = adapted_in_weight
                base_layer.out_proj.get_base_layer().weight.data = adapted_out_weight
                
                # Run forward pass with adapted weights
                result = base_layer(x, *args, **kwargs)
            finally:
                # Restore original weights
                base_layer.in_proj_weight.data = temp_in_weight
                base_layer.out_proj.get_base_layer().weight.data = temp_out_weight
        elif self.merged:
            result = self.base_layer(x, *args, **kwargs)
        else:
